...
impl OvalDefinitions {
    ...
    pub fn merge(&mut self, other: OvalDefinitions) -> Result<()> {
        ...
        if let Some(other_states) = other.states.rpminfo_states {
            ...
            let mut merged_states = self.states.rpminfo_states.take().unwrap_or_else(Vec::new);
        ...
        if let Some(other_verify_states) = other.states.rpmverifyfile_states {
            ...
            let mut merged_verify_states = self.states.rpmverifyfile_states.take().unwrap_or_else(Vec::new);
...
///
/// 返回最高严重性级别的字符串表示
pub fn calculate_max_severity(cves: &Vec<CVE>) -> String {
    if cves.is_empty() {
